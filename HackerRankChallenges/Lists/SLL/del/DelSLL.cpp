...
namespace sll {
	...
	s_ptr Erase(s_ptr head, size_t index, size_t count) {
		if (not head) return nullptr;
		s_ptr dummy = std::make_shared<Node>(0);
		s_ptr dummy_begin = dummy;
		s_ptr current = head;
		...
		dummy->next = current ? current : nullptr;
		dummy_begin = dummy_begin->next;
		return dummy_begin;
	}
	...
	s_ptr EraseBetween(s_ptr head, size_t begin, size_t end) {
		...
		if (end > begin)
			return Erase(head, begin, end - begin);
		else
			return Erase(head, end, begin - end);
	}

	s_ptr Clear(s_ptr head, size_t from) {
		if (not head) return nullptr;
		size_t counter = 0;
		s_ptr current = head;
		s_ptr prev;
